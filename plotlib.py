...
def _plot_profile(x, y, Ra, arclength, pr_number, pr_exp, xlabel, ylabel, xlim=[0,2], ylim=[0,1.1], title=None):
    Ra_at_L = Ra[-1:]
...
def plot_heat(xi_q, q_norm, Ra, arclength, pr_number, pr_exp=0.2):
...
def plot_current(xi_J, J_norm, Ra, arclength, pr_number, pr_exp=0.3):
    return _plot_profile(
...
def plot_pressure(xi_P, P_norm, Ra, arclength, pr_number, pr_exp=0.8):
    return _plot_profile(
        xi_P, P_norm, Ra, arclength, pr_number, pr_exp,
        xlabel=r"$(R/\sqrt{R_a L})\,\mathrm{Pr}^{0.8}$",
        ylabel=r"$P/P_{\max}$",
        xlim=[0,1.5],
        title="Arc pressure profile"
    )

def plot_shear(xi_S, S_norm, Ra, arclength, pr_number, pr_exp=0):
    return _plot_profile(
        xi_S, S_norm, Ra, arclength, pr_number, pr_exp,
        xlabel=r"$R/\sqrt{R_a L}$",
        ylabel=r"$S/S_{\max}$",
        xlim=[0,2.5],
        title="Shear stress profile"
    )
...
def plot_normalized_profiles(arc_df, Pr_number):
    """
    Grafica:
      - B/Bmax° vs Xcoord1
      - T/Tmax° vs Xcoord2
    ...
    ax1.vlines(1/(Pr_number**0.5), 0, 1.1, linewidth=2, color="r", label="R = $R_a$")
    ax1.set_xlabel(r"$R/Ra*Pr^{-0.5}$")
    ax1.set_ylabel(r"$B/B_{max}^\circ$")
    ax1.set_ylim(0,1.1)
    ax1.set_xlim(0,8)
    ax1.set_title("Magnetic field profiles")
    ...
    ax2.vlines(1/Pr_number, 0, 1.1, linewidth=2, color="r", label="R = $R_a$")
    ax2.vlines(0, 0, 1.1, linewidth=1.5, color="blue", linestyles="--")
    # ax2.vlines(1/Pr_number**2, 0.02, 1.1, linewidth=1.5, color="blue", linestyles="--", label="Range of validity")
    ...
    ax3.vlines(1/Pr_number, 0, 1.1, linewidth=2, color="r", label="R = $R_a$")
    ax3.set_xlabel(r"$R/Ra*Pr^{-1}$")
    ax3.set_ylabel(r"$V/V_{max}^\circ$")
    ax3.set_xlim(0,5)
    ax3.set_ylim(0,1.1)
    ax3.set_title("Axial velocity profiles")
